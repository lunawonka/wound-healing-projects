...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for x, y in train_bar:
            ...
            train_bar.set_postfix(loss=f"{loss.item():.4f}", recon=f"{recon.item():.4f}", kl=f"{kl.item():.4f}")
        ...
        with torch.no_grad():
            for x, y in val_bar:
                ...
                val_bar.set_postfix(loss=f"{loss.item():.4f}", recon=f"{recon.item():.4f}", kl=f"{kl.item():.4f}")
...
#run B
    #CUDA_VISIBLE_DEVICES=6 python train_cvae_exp.py --run_name cvae_patch256_ld128_beta5e-4_bs32_augfliprot_e80 --beta 5e-4 --epochs 80 --latent_dim 128 --cond_dim 32 --batch_size 32 --lr 1e-4 --weighted_sampler
