...
def parse_args():
    ...
    parser.add_argument("--msssim_weight", type=float, default=0.84)
...
def mixed_recon_loss(x_hat, x, l1_weight=0.16, msssim_weight=0.84):
    # input to ms-ssim should be in [0,1]
    x_hat_01 = denorm(x_hat).clamp(0, 1)
    x_01 = denorm(x).clamp(0, 1)

    l1 = F.l1_loss(x_hat, x, reduction="mean")
    msssim_val = ms_ssim(x_hat_01, x_01, data_range=1.0, size_average=True)
    msssim_loss = 1.0 - msssim_val

    recon = l1_weight * l1 + msssim_weight * msssim_loss
    return recon, l1, msssim_loss, msssim_val


def vae_loss(x_hat, x, mu, logvar, beta=5e-4, l1_weight=0.16, msssim_weight=0.84):
    recon, l1, msssim_loss, msssim_val = mixed_recon_loss(
        x_hat, x, l1_weight=l1_weight, msssim_weight=msssim_weight
    )
    kl = -0.5 * torch.mean(1 + logvar - mu.pow(2) - logvar.exp())
    total = recon + beta * kl
    return total, recon, kl, l1, msssim_loss, msssim_val
...
def main():
    ...
    config["recon_loss"] = "0.84*(1-MS-SSIM)+0.16*L1"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_l1_sum = train_msssim_loss_sum = train_msssim_val_sum = 0.0
        ...
        for x, y in train_bar:
            ...
            loss, recon, kl, l1, msssim_loss, msssim_val = vae_loss(
                x_hat, x, mu, logvar,
                beta=args.beta,
            ...
            train_l1_sum += l1.item()
            train_msssim_loss_sum += msssim_loss.item()
            train_msssim_val_sum += msssim_val.item()
        ...
        train_msssim_loss = train_msssim_loss_sum / len(train_loader)
        train_msssim_val = train_msssim_val_sum / len(train_loader)

        model.eval()
        val_loss_sum = val_recon_sum = val_kl_sum = 0.0
        val_l1_sum = val_msssim_loss_sum = val_msssim_val_sum = 0.0
        ...
        with torch.no_grad():
            for x, y in val_bar:
                ...
                val_kl_sum += kl.item()
                val_l1_sum += l1.item()
                val_msssim_loss_sum += msssim_loss.item()
                val_msssim_val_sum += msssim_val.item()
        ...
        val_msssim_loss = val_msssim_loss_sum / len(val_loader)
        val_msssim_val = val_msssim_val_sum / len(val_loader)
        ...
        logger.info(
            ...
            f"l1={train_l1:.4f} msssim_loss={train_msssim_loss:.4f} msssim={train_msssim_val:.4f} | "
            f"val_loss={val_loss:.4f} recon={val_recon:.4f} kl={val_kl:.4f} "
            f"l1={val_l1:.4f} msssim_loss={val_msssim_loss:.4f} msssim={val_msssim_val:.4f} | "
        ...
        history.append({
            ...
            "train_kl": train_kl,
            "train_l1": train_l1,
            "train_msssim_loss": train_msssim_loss,
            "train_msssim": train_msssim_val,
            "val_loss": val_loss,
            "val_recon": val_recon,
            "val_kl": val_kl,
            "val_l1": val_l1,
            "val_msssim_loss": val_msssim_loss,
            "val_msssim": val_msssim_val,
